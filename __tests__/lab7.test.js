describe('Basic user flow for Website', () => {
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    console.log(`Checking product items`);
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const text = await buttonHandle.getProperty('innerText');
    const inner = await text.jsonValue();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 0; i < items.length; i++){
      ...
      const text = await buttonHandle.getProperty('innerText');
      const inner = await text.jsonValue();
      if (inner != 'Remove from Cart') allPass = false;
    }
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    const text = await cartCountHandle.getProperty('textContent');
    const innerText = await text.jsonValue();
    if(innerText != '20') allPass = false;
    expect(allPass).toBe(true);
  }, 20000);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for (let i = 0; i < items.length; i++){
      ...
      const text = await buttonHandle.getProperty('innerText');
      const inner = await text.jsonValue();
      if (inner != 'Add to Cart') allPass = false;
    }
    const cartCountHandle = await page.$('#cart-count');
    const text = await cartCountHandle.getProperty('textContent');
    const innerText = await text.jsonValue();
    if(innerText != '0') allPass = false;
    expect(allPass).toBe(true);
